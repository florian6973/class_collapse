...
def plot_classification(config, model, data, comment):
    ...
    plt.subplot(2,2,2)
    plt.scatter(data_emb[:,0], data_emb[:,1], c=data.y_test)
    ...
    plt.title(f"KNN accuracy: {np.mean(y_pred_fine == knn.data.y_test_fine):.3f} ({comment})")


    plt.subplot(2,2,4)
    plt.scatter(data.X_test_embs[:,0], data.X_test_embs[:,1], c=y_pred_coarse)    
    plt.title(f"KNN accuracy: {np.mean(y_pred_coarse == knn.data.y_test):.3f} ({comment})")
